...
# Fix Unicode characters that break LaTeX
table1_df$Characteristic <- gsub("²", "$^2$",  table1_df$Characteristic)
table1_df$Characteristic <- gsub("≥", "$\\\\geq$", table1_df$Characteristic)

kable(table1_df,
      row.names = FALSE,
      booktabs  = TRUE,
      escape    = FALSE,
      caption   = "Baseline Characteristics of Analytic Sample by Hard Drug Use at Baseline. Mean (SD) for continuous variables; n (\\%) for categorical variables.",
      col.names = c(
        "Characteristic",
...
```{r table4-5, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
############################################################################
...
# Open landscape environment manually
cat("\\begin{landscape}\n")

# Table 4 - Frequentist Mediation
kable(med_table,
      row.names = FALSE,
  ...
  kable_styling(latex_options = c("striped", "scale_down"),  
                full_width    = F) %>%    # remove hold_position in landscape
...
This analysis examined whether treatment response two years after HAART initiation differed between HIV-positive men who reported hard drug use at baseline and those who did not, across four outcomes: viral load, CD4+ T cell count, physical quality of life, and mental quality of life. After adjustment for baseline outcome values, age, BMI, smoking status, education, and race/ethnicity, hard drug users demonstrated significantly lower CD4+ T cell counts and worse physical quality of life at Year 2 compared to non-users. These differences were clinically meaningful, with hard drug users averaging approximately 164 fewer CD4+ T cells, well exceeding the 50-cell clinical threshold, and demonstrating worse physical quality of life on the reflected log scale. No significant differences were observed for viral load or mental quality of life. Results were highly consistent between the frequentist and Bayesian frameworks, lending additional confidence to these findings.

The observation that hard drug use was associated with impaired CD4 recovery but not with viral load suppression is clinically interesting. It suggests that while hard drug users may achieve comparable virologic suppression under HAART, their immune reconstitution remains compromised, consistent with laboratory evidence that hard drugs may directly inhibit immune function independent of viral replication. The significant association with physical but not mental quality of life may reflect the direct physiological consequences of hard drug use on physical health, while mental health trajectories may be driven by other factors not captured in this analysis.

Contrary to our hypothesis, medication adherence did not appear to mediate the relationship between hard drug use and treatment outcomes. Both frequentist and Bayesian mediation analyses found that the indirect effect through adherence was small and not statistically significant for any outcome, with the vast majority of the total effect operating through direct pathways. This suggests that the poorer immunologic and physical outcomes observed in hard drug users are not simply a consequence of missed doses, and that interventions focused solely on improving adherence may be insufficient to close the gap in treatment response between hard drug users and non-users.

Several limitations of this analysis warrant consideration. First, the hard drug user group was small, comprising only 36 of 463 subjects (7.8%), which substantially limits statistical power to detect moderate associations and increases the width of confidence intervals, particularly for viral load and quality of life outcomes. The investigator should interpret non-significant findings with caution, as they may reflect insufficient power rather than a true absence of effect. Second, hard drug use was defined only at baseline, and subjects may have changed their drug use behavior over the two-year follow-up period. Misclassification of exposure over time could bias estimates toward the null. Third, the analysis relied on complete case analysis, excluding subjects with missing data on any outcome or covariate, which could introduce bias if missingness was related to drug use status or health trajectory. Fourth, the MACS cohort consists exclusively of homosexual and bisexual men, limiting generalizability to other populations affected by HIV, including women and heterosexual men. Finally, while we adjusted for several important confounders, residual confounding from unmeasured variables such as other substance use, mental health diagnoses, and social support cannot be ruled out.

These findings suggest that baseline hard drug use is associated with meaningful deficits in immunologic recovery and physical quality of life two years after HAART initiation, independent of medication adherence. Clinicians treating HIV-positive patients who use hard drugs may need to consider more aggressive immunologic monitoring and targeted interventions beyond adherence support to improve treatment outcomes in this population.


# Reproducible Research

GitHub Link: https://github.com/asteinwand/BIOS6624.git

File Path to data: hivdat <- read.csv("C:/Users/stein/OneDrive/Documents/School/2026 Spring/Advanced Methods/BIOS6624/Project 1/Data Raw/hiv_6624_final.csv") 
...
## Subject Flow
```{r subject-flow, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', out.width='80%'}
knitr::include_graphics("P1 N Tracking (1).pdf")
```

## MCMC Trace Plots
```{r trace-plots, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=8, fig.align='center', out.width='\\textwidth'}

# 4 outcomes, showing hard drug coefficient (beta[2]) and sigma
params_trace <- c("beta[2]", "sigma")

# Viral Load
draws_vload <- as_draws_array(result_vload$fit$draws())
mcmc_trace(draws_vload, pars = params_trace) +
  ggtitle("Trace Plots: Viral Load (log10)")

# CD4 Count
draws_cd4 <- as_draws_array(result_cd4$fit$draws())
mcmc_trace(draws_cd4, pars = params_trace) +
  ggtitle("Trace Plots: CD4 Count")

# Physical QoL
draws_phys <- as_draws_array(result_phys$fit$draws())
mcmc_trace(draws_phys, pars = params_trace) +
  ggtitle("Trace Plots: Physical QoL (reflected log)")

# Mental QoL
draws_ment <- as_draws_array(result_ment$fit$draws())
mcmc_trace(draws_ment, pars = params_trace) +
  ggtitle("Trace Plots: Mental QoL (reflected log)")
```
